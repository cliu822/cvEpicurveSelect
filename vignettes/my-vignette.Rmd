...
#### Visualize the in-sample predicted vs target
...
data.frame(Week = 1:length(goal_vec),
  ...
  labs(title = "Actual vs. Predicted (Cali, Colombia)",
       x = "Week",
       y = "Cases") +
...
col_adm2_sf <- col_adm2_sf%>%
  mutate(NAME_1 = toupper(NAME_1),
         ...
         NAME_2 = toupper(gsub("\\s+", "", NAME_2)))%>%
  mutate(NAME_1 =
           case_when(NAME_1 == "BOGOTA D.C."~"BOGOTA",
                     NAME_1 == "LA GUAJIRA"~"GUAJIRA",
                     NAME_1 == "NORTE DE SANTANDER"~"NORTE SANTANDER",
                     NAME_1 == "SAN ANDRES Y PROVIDENCIA"~"SAN ANDRES",
                     NAME_1 == "VALLE DEL CAUCA" ~"VALLE",
                     TRUE ~NAME_1),
         NAME_2 = ifelse(NAME_2 == "SANTIAGODECALI","CALI", NAME_2))
...
col_adm2_sf <- col_adm2_sf%>%
 ...
 ggplot() +
  ...
  scale_fill_manual(na.value = "white",values= c("Target"="darkblue", "Control"="pink")) +
  theme_minimal() +
  theme(
...
fit_pre <- lm(y[t_pre] ~ as.matrix(X[t_pre, , drop = FALSE])) 
w <- coef(fit_pre)
w[is.na(w)] <- 0
